...
def test_product_facade_observation_map_defines_lab_without_contracting_lifecycle():
    ...
    for line in (
        ...
        "Product facade response observations are still lab evidence",
        "required_actions",
        "receipt_handle",
        "replayable_now",
        "audit_pending",
        ...
        "`required_actions` should be user-facing product language",
        "`touch_log` remains lab-only diagnostic material",
        "## Comp-Compatible Verification Input Observations",
        "not a final export bundle format",
        ...
        "must not export a replay report as if that report were product authority",
        "`ProjectionReplayReport` is produced by the comp verifier",
        "receipt authenticity separately from replay",
        "`CompCompatibleVerificationInput`",
        "It omits product-only workflow state, `touch_log`, proof graph output, and `ProjectionReplayReport`.",
...
def test_product_facade_conformance_runner_map_positions_next_layer():
    ...
    for line in (
        ...
        "suite summary remains lab-only reporting",
